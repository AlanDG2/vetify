...
test.describe('Gestión de Usuario Test Suite', () => {
    ...
    // =========================================================================
    // CATEGORY: TS-04 IMAS-3216 - Olvidé contraseña (CP01-CP05, reusa el diseño de
    // docs/user-stories/IMAS-3215-reseteo-contrasena-b2c-vetify.tests.md — confirmado en
    // docs/user-stories/IMAS-3216-reseteo-contrasena-capitado-osde.md que OSDE Capitado comparte la
    // misma pantalla /auth/login y el mismo VetifyWebappLoginPage que Vetify B2C, solo cambia el pool
    // de usuarios (SiteId.OSDE_CAPITADO). CP06-CP09 (contenido/recepción del email) siguen bloqueados
    // por falta de infraestructura de lectura de inbox (IMP-006).
    // =========================================================================
    test.describe('TS-04 IMAS-3216 - Olvidé contraseña', () => {
        test.beforeEach(async ({ container }) => {
            await container.vetify.webapp.loginPage.load();
    ...
    // =========================================================================
    // CATEGORY: TS-05 IMAS-3216 - Cambio de contraseña (tras el link del correo)
    // =========================================================================
    // Cuenta real dedicada (2026-09-01, `alandgg972@gmail.com`) — a diferencia de Vetify B2C, este
    // producto no tenía ninguna cuenta REAL_EMAIL (bloqueaba IMAS-3480). Mismo diseño que
    // `vetify-b2c/user-management.spec.ts` TS-05, solo cambia SiteId/cuenta/casilla IMAP.
    test.describe('TS-05 IMAS-3216 - Cambio de contraseña', () => {
        test.describe.configure({ mode: 'serial' });

        test('TC-01 - OSDE Capitado - Redirección al link del correo y campos de contraseña obligatorios', { tag: ['@critical'] }, async ({ container, page }) => {
            const user = await UserProvider.getUser({ source: UserSource.Pooled, siteId: SiteId.OSDE_CAPITADO, tags: [UserTag.REAL_EMAIL], reserve: true });
            test.skip(!user, 'No hay una cuenta con casilla de correo real disponible (UserTag.REAL_EMAIL).');
            const mailbox = getRealMailboxCredentials(SiteId.OSDE_CAPITADO);
        ...
        test('TC-02 - [Negativo] OSDE Capitado - Rechazo de contraseña débil', { tag: ['@critical'] }, async ({ container, page }) => {
            const user = await UserProvider.getUser({ source: UserSource.Pooled, siteId: SiteId.OSDE_CAPITADO, tags: [UserTag.REAL_EMAIL], reserve: true });
            test.skip(!user, 'No hay una cuenta con casilla de correo real disponible (UserTag.REAL_EMAIL).');
            const mailbox = getRealMailboxCredentials(SiteId.OSDE_CAPITADO);
        ...
        test('TC-03 - [Negativo] OSDE Capitado - Contraseñas no coinciden', { tag: ['@critical'] }, async ({ container, page }) => {
            const user = await UserProvider.getUser({ source: UserSource.Pooled, siteId: SiteId.OSDE_CAPITADO, tags: [UserTag.REAL_EMAIL], reserve: true });
            test.skip(!user, 'No hay una cuenta con casilla de correo real disponible (UserTag.REAL_EMAIL).');
            const mailbox = getRealMailboxCredentials(SiteId.OSDE_CAPITADO);
        ...
        test('TC-04 - OSDE Capitado - Cambio exitoso, login con la nueva contraseña, la anterior invalidada y el link reusado rechazado', { tag: ['@critical'] }, async ({ container, page }) => {
