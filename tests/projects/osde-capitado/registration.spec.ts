import { setAllureDetails, test } from '../../framework/osde-base-test';
...
test.describe('Registración y Adquisición Test Suite', () => {
    test.describe('TS-01 Flujo de Compra', () => {
        test('CP-01 - Flujo de compra - Nuevo usuario adquiriente - Compra existosa - Plan individual', async ({ institutionalPage, checkoutPage, page }) => {
            ...
            await institutionalPage.load();
            await institutionalPage.plans.scrollIntoView();
            const selectedPlan = await institutionalPage.plans.contractRandomPlan();

            await checkoutPage.expectSelectedPlan(selectedPlan);

            await checkoutPage.completePersonalData({
            ...
            await checkoutPage.completeBillingData({
            ...
            await checkoutPage.completePaymentData({
        ...
        test('CP-02 - Flujo de compra - Nuevo usuario adquiriente - Compra fallida', async ({ institutionalPage, checkoutPage, page }) => {
