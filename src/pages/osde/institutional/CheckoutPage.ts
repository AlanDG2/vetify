...
export class OsdeAdquirienteCheckoutPage extends BasePage {
    ...
    async completePersonalData(data: PersonalData): Promise<string> {
        ...
        await this.page.waitForURL(/\/checkout\/billing\?from=osde.*$/);
    ...
    async completeBillingData(data: BillingData): Promise<void> {
        ...
        await this.page.waitForURL(/\/checkout\/payment\?from=osde.*$/);
    }
