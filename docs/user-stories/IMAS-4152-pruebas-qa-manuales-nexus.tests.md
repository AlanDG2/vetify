...
**CP02 - Verificar alta cuando la póliza real no está cargada en el datalake de Nexus QA** ✅✅ **EJECUTADO 2026-08-22 — NO PASA COMO SE ESPERABA (ver nota)**
- Dado: cuenta Cruella Devil (DNI `98765499`, mascota Manchitas), intentando dar de alta un reintegro con categoría **"Traslado de mascotas"** (`clSubServicio=502`).
- Cuando: se envía la solicitud de alta.
- Entonces (esperado por código): `resolveCapabilityId` debería caer al fallback de catálogo fijo (`TEMP_CAPABILITY_CATALOG_POLICY_KEY`) y completar el alta **sin error visible al usuario**, transparente.
- **Resultado real**: `POST .../solicitudes/confirmacion` → **502 Bad Gateway**, código `BUS-006` — *"Nexus claimsHistory does not report capability for clSubServicio=502"*. El alta **no se completó** — el usuario ve un error explícito (no el fallback silencioso esperado). Reproducido en vivo por Alan durante la sesión de reproducción de `BUG-015`.
- **Por qué "no pasa como se esperaba" y no directamente "falla"**: el gap de fondo (póliza/capability no sincronizada en el datalake de Nexus QA) es un límite ya conocido y documentado (`docs/conocimiento-sistema.md`) — lo que no coincide con lo esperado es el **comportamiento ante ese gap**: para esta categoría puntual, el fallback no se disparó (o no cubre este `clSubServicio`) y en vez de eso explotó con un 502. Vale la pena que dev confirme si el fallback cubre todas las categorías o solo algunas — documentado como hallazgo, ver también la nota sobre esto en `docs/bugs/BUG-015-...md`.
- Trazabilidad: **regresión del fix `#110`** — parcialmente confirmada (el mecanismo de fallback existe y se probó antes con éxito para otras categorías, pero no para `clSubServicio=502` en esta corrida).
...
|---|---|---|
| `IMAS-4103` (Alta) | CP01 ✅, CP02 ⚠️, CP11 🔴 | Parcial — CP01 pasó en vivo; CP02 ejecutado pero con resultado discrepante (ver nota) |
...
**11 casos diseñados. 4 ejecutados y pasando en vivo (CP01, CP03, CP07, CP08 — 2026-08-22)**, contra QA real, con VPN conectada, en 4 solicitudes distintas (`3184-1`: alta→pago; `3186-1`: alta→Calidad aprueba→Finanzas rechaza (Inconveniente)→Calidad rechaza definitivo; `3188-1` y `3192-1`: intentos de rechazo directo desde `PENDIENTE`, ambos bloqueados). Se intentó específicamente ejercer el camino de rechazo directo de Calidad (**CP04/CP05**), y se encontró un bloqueo real y reproducible **2 veces de forma independiente** (dos errores de backend en cadena, `400 BUS-009` y `404 BUS-005`), documentado en `docs/bugs/BUG-015-rechazo-directo-calidad-pendiente-requiere-clcuenta.md` — ya trackeado en Jira como `IMAS-4354` (dev, "En Progreso"), confirmado 2026-08-24. **CP02 también se ejecutó** (categoría "Traslado de mascotas") pero con resultado discrepante: en vez del fallback silencioso esperado, devolvió `502/BUS-006` — documentado como hallazgo, no bloqueante. CP06 y CP09 quedaron parcialmente confirmados: el 204 de `registro-pago`/`decision-calidad` prueba que el flujo de negocio cerró bien, pero la nota específica en Nexus (best-effort, servidor-a-servidor) sigue sin verificación visual — requiere el panel interno de Nexus (CP11), sin credenciales en esta sesión. CP10 requiere fault-injection no disponible manualmente. **Esta HU NO puede reportarse como cerrada/lista todavía** — CP04/CP05 quedaron bloqueados, CP02 tiene un resultado a revisar, y falta la confirmación visual en el panel interno de Nexus (CP06/CP09/CP11).
