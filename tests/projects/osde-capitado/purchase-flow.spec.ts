    ...
    test.describe('TS-01 Proceso de Compra', () => {
        test('TC-01 Canje de cupon exitoso', async ({ container, page }) => {
            ...
            await step('1. Cargar la landing Page', async () => {
                await container.osdeCapitado.landingPage.load();
            });
            await step('2. Cargar todos los datos requeridos del formulario de compra', async () => {
                await container.osdeCapitado.landingPage.form.completeForm({
            ...
            await step(`3. Ingresar un cupon que no se haya usado nunca ["${cupon!.code}"]`, async () => {
                await container.osdeCapitado.landingPage.form.cuponInput.fill(cupon!.code);
            });
            await step('4. Presionar el botón "ACTIVA SU PLAN"', async () => {
                const response = await container.osdeCapitado.landingPage.form.submitForm();
                const responseData = await response.json();
                expect.soft(response.status()).toBe(201);
                expect.soft(responseData).toHaveProperty('id');
                await expect.soft(page).toHaveURL(environment.OSDE_CAPITADO_INSTITUTIONAL_BASE_URL);
        ...
        test('TC-02 Canje de cupon no válido', async ({ container, page }) => {
            ...
            await step('3. Ingresar un cupón no válido', async () => {
                await container.osdeCapitado.landingPage.form.cuponInput.fill('TEST');
            });
            await step('4. Presionar el botón "ACTIVA SU PLAN"', async () => {
                const response = await container.osdeCapitado.landingPage.form.submitForm();
                expect.soft(container.osdeCapitado.landingPage.form.errorMessageLbl).toHaveText('Ese cupón no es válido.');
                expect.soft(page).toHaveURL(environment.OSDE_CAPITADO_INSTITUTIONAL_BASE_URL);
        ...
        test('TC-03 Canje de cupon ya usado', async ({ container, page }) => {
            ...
            await step('3. Ingresar un cupon válido pero ya usado', async () => {
                await container.osdeCapitado.landingPage.form.cuponInput.fill('TESTOSDE734292');
            });
            await step('4. Presionar el botón "ACTIVA SU PLAN"', async () => {
                const response = await container.osdeCapitado.landingPage.form.submitForm();
                expect.soft(container.osdeCapitado.landingPage.form.errorMessageLbl).toHaveText('Ese cupón ya fue utilizado.');
                expect.soft(page).toHaveURL(environment.OSDE_CAPITADO_INSTITUTIONAL_BASE_URL);
    ...
    test.describe('TS-02 Validación de formulario', () => {
        test('TC-04 Campos obligatorios', async ({ container, page }) => {
            ...
            await step('2. Presionar el botón "ACTIVA SU PLAN" sin completar ningún campo', async () => {
                await container.osdeCapitado.landingPage.form.submitButton.click();
            });
            await step('El sistema no envía el registro y no navega fuera de la landing', async () => {
                expect(page.url()).toContain(environment.OSDE_CAPITADO_INSTITUTIONAL_BASE_URL);
            });
        });
        ...
        test('TC-05 Tipos de documentos', async ({ container }) => {
            ...
            await step('El selector muestra al menos una opción de tipo de documento válida (ej. DNI)', async () => {
                const options = await container.osdeCapitado.landingPage.form.documentTypeSelect.locator('option').allTextContents();
