...
export class VetifyWebAppPolicyValidationPage extends VetifyWebAppBasePage {
    ...
    async validatePolicy(data: PolicyValidationData, options?: {
        updateForSignupResponseStatus: number
    }): Promise<void> {

        const {
            updateForSignupResponseStatus = 200,
        } = options ?? {};
        ...
        await Promise.all([
            this.page.waitForResponse(response =>
                response.url().includes('/api/users/update_for_signup') && response.status() === updateForSignupResponseStatus),
