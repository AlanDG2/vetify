...
- **Login / Registro / Validación de póliza**: `LoginPage`, `RegistrationPage`, `PolicyValidationPage` — el registro pide DNI y lo valida contra la póliza antes de dejar completar alta.
  - **Reseteo de contraseña (IMAS-3215, automatizado 2026-08-08 en `tests/projects/vetify-b2c/user-management.spec.ts` TS-04, POM `LoginPage` extendido)**:
    ...
    - **Bug corregido 1 — `IMAS-4198`** (reportado a Jira 2026-08-07, confirmado resuelto en vivo 2026-08-31): antes, campo vacío → backend respondía bien (400, `"email es requerido"`), pero el frontend ignoraba ese mensaje y mostraba el texto genérico de "problemas técnicos" ("En este momento estamos con problemas técnicos... 0800-122-6238") en vez de indicar que falta el campo. **Ahora** valida en el cliente antes de llamar al backend: marca el campo inválido con "El correo electrónico no es válido", sin request a `/api/passrecovery`.
    - **Bug corregido 2 — `IMAS-4199`** (reportado a Jira 2026-08-07, confirmado resuelto en vivo 2026-08-31): antes no había validación de formato de email en este campo — `"noesunemail"` era aceptado (200 OK, mismo mensaje de éxito). **Ahora** usa el mismo validador/mensaje que `RegistrationPage` ("El correo electrónico no es válido"), sin llamar al backend. Mismo fix del lado del cliente resuelve ambos bugs a la vez — ver retest completo en `docs/user-stories/IMAS-3215-reseteo-contrasena-b2c-vetify.tests.md`.
    - **OSDE Adquirente, OSDE Capitado y Flux Capitado comparten literalmente la misma pantalla** `/auth/login` y el mismo `VetifyWebappLoginPage` (confirmado por los 3 en `docs/user-stories/IMAS-3216-*.md`/`IMAS-3217-*.md` y por corrida real) — el mismo diseño de casos se replicó a los 3 solo cambiando `siteId` del pool (`tests/projects/{osde-adquirente,osde-capitado,flux-capitado}/user-management.spec.ts` TS-04). Para Flux Capitado, CP02 usa tag `ACTIVE` en vez de `REGISTERED` porque el pool de ese producto solo tiene 1 usuario disponible tageado así — un `ACTIVE` sigue siendo un email existente, que es lo único que ese caso necesita validar. El fix de IMAS-4198/IMAS-4199 se verificó y quedó automatizado en los 4 (20/20 verdes, `TS-04` completo).
    - **Bloqueo de automatización end-to-end (parcial, ya no aplica a los 4 productos por igual)**: no hay infraestructura de lectura de casilla de correo para **OSDE Capitado ni Flux Capitado** (confirma IMAS-3476/IMAS-3480, subtareas de IMAS-3216/IMAS-3217, siguen en Backlog — sin cuenta `UserTag.REAL_EMAIL` en sus pools) — para esos 2 productos no se puede automatizar más allá del envío de la solicitud (TS-04). **Vetify B2C sí tiene la infraestructura** (`IMP-006` resuelto, cuenta real `alan.gonzalez@ingenia.la`) — el flujo completo (recepción/contenido del email, link de reset, cambio de contraseña, re-login) está automatizado en `TS-05 IMAS-3215 - Cambio de contraseña` del mismo spec. Ver `docs/impedimentos-bloqueos.md` IMP-006 y diseño de casos completo en `docs/user-stories/IMAS-3215-reseteo-contrasena-b2c-vetify.tests.md`.
...
- **Banner de beneficios en Home — "Vetify PLUS" / "Cooper" (IMAS-4356, verificado en vivo 2026-08-26/27)**: la pantalla de Home tiene un banner de "descuentos y beneficios" (también replicado como entrada propia en el menú lateral, sección "Cuenta") que **varía según el segmento y el estado del perfil del usuario**, no es fijo:
  ...
  - **⚠️ Lo de arriba quedó parcialmente desactualizado por `IMAS-4435`** ("Implementación de Banner Cooper con mayor protagonismo", el seguimiento directo de IMAS-4356 para Vetify B2C — la "otra card" que esta última mencionaba). **Confirmado en vivo 2026-08-31** con la misma cuenta Vetify B2C de siempre (`user_1785886357504_7fce46f5`): el banner de Home ahora sí muestra Cooper para B2C también ("Beneficio exclusivo clientes Vetify", botón "Ir a Cooper") — ya no es solo Vetify PLUS. Y en el menú lateral, "Cuenta" ya no tiene el ítem de beneficios: existe una **sección nueva dedicada "Beneficios"**. Arquitectura (`IMAS-4449`) y Diseño (`IMAS-4448`) están Hecho y ya deployados en QA; Desarrollo (`IMAS-4436`) también Hecho. **Falta 100% del lado QA**: `IMAS-4437` (Pruebas QA, subtarea de `IMAS-4435`) sigue en Backlog, sin ningún caso de prueba diseñado todavía — nada de esto tiene CP en `Casos de Prueba.xlsx` ni HU local en `docs/user-stories/` aún.
  - **Reglas de negocio confirmadas por el dev (Juan Cruz Triventi, vía Alan, 2026-08-31) para `IMAS-4435`** — el dev aclaró que la sesión anterior probó esto **antes de que el despliegue real terminara**, y dio la regla completa: (1) todos los usuarios ven el banner Cooper en Home; (2) todos excepto OSDE ven **Cooper y Vetify PLUS** en el menú; (3) los de OSDE ven **solo Cooper** en el menú (sin Vetify PLUS).
  - **⚠️ Trampa real de metodología, ya corregida**: el primer re-test (login secuencial a los 3 segmentos en la misma pestaña, sin limpiar nada entre medio) dio "OSDE Capitado y OSDE Adquirente muestran Cooper + Vetify PLUS los dos" — Alan reportó que en su propia prueba **OSDE Capitado mostraba solo Cooper**, contradiciendo eso. Causa raíz confirmada: el **Service Worker (Workbox) cachea las respuestas de `/api/*`** (bucket `apis`, ver sección de "Mensajes por sistema caído" más abajo) **con una clave que no varía por usuario** — cambiar de cuenta con logins sucesivos en la misma pestaña, sin `unregister()` del SW ni `caches.delete()` entre cada uno, sirve la respuesta de segmentación de la cuenta ANTERIOR a la cuenta nueva. Se repitió cada login limpiando `navigator.serviceWorker.getRegistrations()` + `caches.keys()`/`.delete()` antes de cada uno — **regla nueva para cualquier prueba futura que compare comportamiento entre cuentas/segmentos en la misma sesión de browser: limpiar SW+caches antes de CADA cambio de cuenta, no solo una vez al principio.**
  - **Re-test en vivo 2026-08-31 (limpio, post-aclaración del dev) contra esas 3 reglas**:
    | Regla | B2C (`7fce46f5`) | OSDE Capitado (`...5615`, "Popi", Grupo 0163/Cod. 2349) | OSDE Adquirente (`...9580`) |
    |---|---|---|---|
    | (1) Banner Cooper en Home | ✅ Cumple | ✅ Cumple | ✅ Cumple (mejora real vs. el gap viejo `BUG-024`/`IMAS-4465`, que ya no reprodujo) |
    | (2)/(3) Menú "Beneficios" | Cooper + Vetify PLUS (correcto) | **Solo Cooper (correcto, confirmado 2 veces con cache limpio + reload)** | **Cooper + Vetify PLUS — no cumple la regla 3 (confirmado 2 veces con cache limpio + reload)** |

    **Hallazgo real (corregido)**: la segmentación del **banner** (regla 1) funciona bien en los 3 segmentos. La del **menú** también funciona bien para **B2C y OSDE Capitado** — el gap es específico de **OSDE Adquirente**, que sigue mostrando "Vetify PLUS" junto con "Cooper" cuando debería mostrar solo Cooper. Coincide con el patrón histórico de que Adquirente y Capitado tuvieron bugs distintos en `IMAS-4356` (`BUG-024` banner vs. `BUG-022` menú) — no es sorprendente que sigan divergiendo. Candidato directo a bug/hallazgo de `IMAS-4437` en cuanto se decida diseñar los casos, acotado a OSDE Adquirente únicamente. Sin reportar a Jira todavía (pendiente de decisión de Alan).
...
- **Menú lateral y navegación** (`SideMenuSection`, `HomePage.openSideMenu()`, verificado en vivo 2026-08-29/30, automatizado en `tests/projects/vetify-webapp/navigation.spec.ts`):
  - El trigger del menú existe como **2 elementos distintos**: `[data-cy="vetifyMenuButton"]` (Desktop) y `[data-cy="vetifyBottomNav-mas"]` (mobile) — solo uno es visible según el viewport. **Bug real encontrado y arreglado**: `openSideMenu()` tenía `isMobile=false` hardcodeado, lo que colgaba 60s en el proyecto "Vetify WebApp Android" (el código intentaba clickear el trigger de Desktop, invisible en mobile). Fix: `Promise.race` entre los dos triggers esperando cuál queda realmente visible.
  - **Solo Home tiene el trigger del menú en su propio header.** Las subpantallas (Mascotas, Perfil, Reintegros, Planes y coberturas) muestran únicamente un botón "Volver" — no hay forma de reabrir el menú desde ahí. El patrón de navegación real es Home → abrir menú → click ítem → volver a Home → repetir, no reabrir el menú desde la subpantalla misma. Confirmado que ningún selector del menú existe/es visible en esas subpantallas (ni siquiera oculto en el DOM, en el caso de `vetifyMenuButton`).
  - **15 ítems reales** en el menú lateral (`SideMenuSection.ts`): Inicio, Mascotas, Perfil, Videollamadas, Veterinarias, Reintegros, Planes y coberturas, Vetify Plus, Emergencias, Asistencia presencial, Asistencia a domicilio, Historial de atención, Ayuda, Coober/Cooper, Facturas, más "Cerrar sesión" (`logoutButton`). **No existe ningún ítem "Atención de Red"** — confirmado también contra la pantalla completa de Servicios (`/section/servicios`, 8 ítems: Emergencias, Veterinarias, Videollamadas, Planes y coberturas, Asistencia presencial, Asistencia a domicilio, Historial de atención, Reintegros). Si el roadmap de la PO lo menciona, es probable mislabeling, no una funcionalidad real sin mapear.
  - **Compatibilidad de permisos por plan**: el ítem "Vetify Plus" aparece en cuentas Vetify Adquirente y **no aparece** en cuentas OSDE Capitado (confirmado con `toBeHidden()`, no solo ausencia de click) — coincide con la regla de negocio ya documentada en la sección de banners de Home.
- **Sesión — login/logout/persistencia** (automatizado 2026-08-29/30 en `tests/projects/vetify-webapp/session.spec.ts`):
  - Acceso directo a una ruta protegida sin sesión redirige a `/auth/login?prevPage=<ruta original>` — preserva la ruta para volver post-login.
  - La sesión persiste entre módulos (navegar Home→Mascotas→Perfil sin volver a pedir login) y ante un `page.reload()` completo.
  - **Logout invalida el `storageState` cacheado de esa cuenta** (`playwright/auth/<id>.json`) — cualquier test que haga logout real debe llamar `UserProvider.clearUserStorageState(user)` después, o el próximo test que reuse esa cuenta del pool aplica cookies muertas y falla de forma confusa (no es un bug de la app, es un detalle de la infraestructura de test).
- **Tour de onboarding** (banner "Comenzar" + 3 pasos, automatizado 2026-08-29/30):
  - Es un flag de **backend por cuenta, no de browser** — una vez que una cuenta completa u omite el tour, no vuelve a aparecer nunca más para esa cuenta (ni borrando cookies/localStorage). Cualquier test que necesite ver el tour por primera vez necesita una cuenta que genuinamente nunca haya logueado antes — **loguearse a mano para "solo mirar" ya consume la cuenta igual que correr el test real** (mismo patrón que `IMP-003`).
  - El botón "Comenzar" (`tourWelcomeStart`) está **duplicado 2x en el DOM** (patrón de resumen responsive ya visto en otras pantallas) — el índice visible **no es estable** (a veces 0, a veces 1). Mismo problema en la pantalla de Ayuda con los textos de "Emergencias"/"Atención al cliente". Solución: filtrar por el pseudo-selector CSS `:visible` en vez de asumir un índice fijo.
- **Diálogo genérico de error ("Algo salió mal / Probá de nuevo en un momento")**: aparece de forma intermitente (mismo patrón de flakiness de backend ya documentado en `IMP-012`/`IMP-014`), con botón "Aceptar" sin `data-cy`. **Mientras está abierto, pone `aria-hidden` en el resto de la página** — el elemento subyacente sigue existiendo y siendo visible en el DOM crudo (`document.querySelector` lo encuentra), pero `getByRole()` de Playwright no, porque respeta `aria-hidden`. Helper `dismissErrorDialogIfPresent()` en `LoggedBasePage` para llamarlo defensivamente antes de asserts basados en rol.
- **Veterinarias — modal de búsqueda** (`VeterinariasSearchModal`, verificado en vivo 2026-08-29/30): el ítem del menú abre un **modal** (no navega a una ruta nueva), título "Buscar veterinaria", con un combobox real de Google Places autocomplete (`aria-label="Ingresá la ubicación"`) y un iframe embebido de Google Maps. **Las sugerencias de ubicación se escriben con `pressSequentially(query, {delay: 100})`, no `.fill()`** — el autocomplete de Google Places no dispara sugerencias con `.fill()`. Los resultados de veterinarias solo se renderizan como pines dentro del iframe cross-origin de Maps (no hay una lista de resultados aparte) — limitación técnica real de Playwright con iframes de terceros, no falta de diseño del caso.
- **Planes y coberturas** (`/section/myplans`, `MyPlansPage`, verificado en vivo 2026-08-29/30): cada plan es un botón-acordeón (nombre real del plan como accessible name, ej. "VETIFY 100 EMERGENCIA x1"). Al expandir se ve "Grupo"/"Código Producto" y un botón **"Condiciones del Servicio"** que abre en pestaña nueva el PDF real del condicionado.
  - **El link del PDF es 100% data-driven desde el backend, no construido en el cliente**: `GET /api/services/plans/engage/{dni}?brand=vetify` devuelve, por cada plan, un campo `pathCondicionado` con la URL completa (ej. `https://www.atencionike.com.ar/pdf/condicionados/0158-2249.pdf`, donde `0158`=`codigoGrupoCuenta` y `2249`=`nroCuenta`). En teoría un plan con `pathCondicionado=null` mostraría el escenario "Plan sin condicionado disponible" — no se encontró ninguna cuenta así en el pool actual (probado en las 5 cuentas con `storageState` cacheado; ver `qa-workspace/decision-log.md` 2026-08-30 para el detalle de por qué no se pudo escanear más a fondo).
  - **Bloqueo técnico real, sin resolver pese a 7 rondas / 4 estrategias distintas**: automatizar la apertura de esa pestaña nueva con el PDF falla de forma reproducible con Playwright (`waitForLoadState()` cuelga porque el visor de PDF nativo de Chromium nunca dispara un evento `load` limpio; `waitForURL()` con `load` y con `commit` también cuelgan; `expect.poll(() => newPage.url())` en un intento devolvió literalmente el string `":"`). La misma interacción funciona perfecto en exploración manual con Playwright MCP — es un problema específico del patrón popup+visor-de-PDF bajo automatización, no falta de esfuerzo. Detalle en `tests/projects/vetify-webapp/plans.spec.ts`.
- **Reintegros — vista del tutor** (`/section/myreintegros`, `ReintegrosPage`, verificado en vivo 2026-08-29/30; distinto del backoffice Calidad/Finanzas documentado en `## Módulo: Reintegros` más abajo): secciones "Cuentas de acreditación" y "Mis reintegros", contadores de estado como texto (`"Pagado (N)"`/`"Solicitado (N)"`/`"Desaprobado (N)"`), botón "Acceder al historial" (deshabilitado si no hay historial). "Nuevo reintegro" abre un wizard de 3 pasos en `/section/nuevo-reintegro`.
  - **RESUELTO 2026-09-10**: el wizard fallaba de forma reproducible en el selector de Mascota (6 cuentas probadas entre Vetify Adquirente y OSDE Capitado) por `GET /api/bff/reintegros/mascotas` respondiendo `400 {"code":"VAL-002","message":"Holder document must be a 7 or 8 digit DNI."}` para cuentas con DNI de otro largo — era `docs/bugs/BUG-007-reintegros-dni-formato-invalido.md` / Jira `IMAS-4279`. **Confirmado en vivo (auditoría de bugs 2026-09-10): IMAS-4279 está en estado "Hecho"**, vinculado a IMAS-4101. Esto destraba el gap más grande de todo el roadmap (todo el módulo de Reintegros — solicitud, carga de documentación, validación de datos, consulta de estado — nunca automatizado como test de Playwright, solo probado manualmente) — ya no hay un bloqueo conocido para empezar a automatizarlo. Pendiente: retestear en vivo con una cuenta de DNI corto para confirmar que el fix sigue sano antes de invertir en la automatización completa.
- **Encuesta de satisfacción post-carga de credencial ("Encuesta CE" en el roadmap de la PO)**: **no existe como feature propia de la app.** Se inspeccionaron en vivo los 19 bundles JS que carga la webapp — el único código relacionado a "survey" es la librería de terceros **Hotjar**; en el código propio de la app (`_app-*.js`) la única llamada a `window.hj(...)` es `hj("identify", userId, {...})` (identificación de usuario para analytics), sin ningún trigger custom atado a completar una credencial. Si esta encuesta existe, es 100% configuración del dashboard de Hotjar (targeting por URL/tiempo), invisible al código de la app y no determinística — no automatizable con Playwright de forma confiable.

**Última actualización de esta sub-sección**: 2026-08-30 (validación final de gaps del roadmap de la PO — ver `qa-workspace/decision-log.md`).
...
**Engage confirmado como sistema real y activo hoy** (no solo un concepto histórico de la épica) — verificado en vivo 2026-08-30 vía `browser_network_requests` contra `vetify-qa.ikeapp.com`: la respuesta de `GET /api/services/plans/engage/{dni}?brand=vetify` trae, por cada plan del cliente, los campos `engageId` y `engageIdProducto` (IDs reales del sistema Engage), además del endpoint dedicado `GET /api/services/plans/suscripciones/engage/{dni}?brand=vetify`. Esto es lo que la PO pide validar en su roadmap bajo la etapa "02. Backend / Engage/ Sise" (`QA-AUTO-005` a `QA-AUTO-015`, 10 flujos de "alta de cliente" y "asociación cliente-plan", todos en 0% porque `Casos de Prueba.xlsx` no tiene ninguna hoja dedicada a este tipo de validación de backend) — pendiente de decisión de alcance de Alan, ver `qa-workspace/decision-log.md`.
...
**🔑 Retomado 2026-09-01 — `BUG-015`/`IMAS-4354` reencuadrado, y hallazgo nuevo potencialmente grave**:

- **La conclusión "sigue roto" del 22/08-28/08 no es válida como estaba planteada.** Retesteado en vivo el mismo expediente `3131739` (VPN reconectada) — falla exactamente igual, mismo `404 BUS-005`, mismo diálogo "Algo salió mal". Pero apareció un comentario del **2026-08-31** en `IMAS-4354` (antes solo-imagen o no leído): el fix solo aplica a expedientes creados **después** de la migración del 26/08 — los expedientes usados en los 3 retests (`3131739`, `3188-1`, `3192-1`, `3131793`) son todos de antes de esa fecha y van a fallar siempre, tenga o no el fix real. **Estado correcto: no probado todavía de forma válida**, no "confirmado roto". Hace falta un expediente genuinamente posterior al 26/08.
- **🔴 Hallazgo nuevo al intentar crear ese expediente**: `/section/nuevo-reintegro` muestra "No hay mascotas registradas para tu documento" en **4 cuentas distintas de 2 productos** (2 Vetify B2C Adquirente + la cuenta Popi de OSDE Capitado, todas con mascota real confirmada en el pasado) — `GET /api/bff/reintegros/mascotas` devuelve `200 []` (array vacío, no el registro-con-campos-null ya documentado el 28/08 — es un síntoma distinto). Más grave: para una de las cuentas, **`GET /api/services/pets/my-products` (el endpoint que alimenta Inicio/Mascotas/Planes/Videollamadas — no específico de Reintegros) también devolvió `[]`**, confirmado visualmente (Inicio sin ninguna tarjeta de mascota). Cache limpiada correctamente antes de cada cuenta (cookies+localStorage+sessionStorage, confirmado por inspección de red). **No confirmado todavía si es un incidente activo del ambiente o algo específico de estas cuentas** — Alan decidió posponer la validación a mañana con el equipo de dev; le dejé una cuenta (`user_1786584481760_8aea8baa@automation.com`) y el paso a paso para que lo confirme de forma independiente. Sin bug filado en Jira todavía. Detalle completo en `docs/user-stories/IMAS-4101-migracion-reintegros-nexus.md` y `qa-workspace/decision-log.md` (2026-09-01).
...
**⚠️ Nota de seguridad**: la colección Postman compartida traía **3 secretos reales hardcodeados**, no solo 2:
1. **Basic Auth real** en `Auth - Get Token` (`pmendoza` / `ikeasistencia`) — sin fecha de vencimiento visible, no confirmado si sigue vigente.
2. **Bearer token JWT real (Auth0 client-credentials)** hardcodeado en el request `pagar-pas` — **decodificado y confirmado VENCIDO** (`iat` 18/05/2026, `exp` 19/05/2026 — ya pasó, sin riesgo actual).
3. **Api-Key real** en el request `Get Client` (`{{IKE_API_BASE_URL}}/cliente`) — sin fecha de vencimiento visible, no confirmado si sigue vigente.

**No se persistieron en este repo** (verificado: el archivo `Vetify.postman_collection.json` en la raíz está sin trackear en git — `git status` lo marca `??`, sin historial). Si el Basic Auth o el Api-Key siguen vigentes, rotarlos; usar siempre variables de entorno (`{{TOKEN}}`, `{{QUANTUM_AUTH_TOKEN}}`) nunca valores pegados directo en un request guardado en un repo compartido. No commitear este archivo tal cual está.

**Ejemplo real — `Get Client` (`{{IKE_API_BASE_URL}}/cliente`)**, para consultar si un DNI ya tiene cliente/plan cargado en el core de Iké (no crea nada, solo consulta):

```bash
curl --location 'https://ike-api-ges-cli-arg-qa.ikeapp.com/cliente' \
--header 'Authorization: Api-Key <API_KEY_REDACTED>' \
...
`{{IKE_API_BASE_URL}}` = `https://ike-api-ges-cli-arg-qa.ikeapp.com` (ambiente QA del core de Iké, distinto del backend Quantum). La Api-Key real vigente está en `Vetify.postman_collection.json` (no trackeado en git, ver nota de seguridad arriba) — no pegarla acá.

**Uso práctico para QA — aislar bugs de checkout sin pasar por la UI completa**: el request `Calculate Payment` (`POST .../payment/calculate?cuenta=<cuenta>`, body `{idTarjeta, esCredito, esDebito, cupon, listInvoicedProducts:[{id, qty}]}`) permite reproducir errores de cálculo de precio (como `IMAS-4431`, "Error al calcular precio del producto") directo contra Quantum, sin checkout completo — útil para probar puntualmente los `clCuenta` de los planes OSDE que fallan (2358/2360/2362/2364) y aislar si el error viene de Quantum mismo o de una capa intermedia (`reintegros-backend`/BFF).
