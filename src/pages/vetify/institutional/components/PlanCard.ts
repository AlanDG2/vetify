...
export class PlanCard {
    ...
    private get root(): Locator {
        return this.plansSection
            ...
            .locator('xpath=ancestor::div[.//button[contains(normalize-space(.),"Contratar")] and count(.//button)=1][1]');
    }
    ...
    async read(): Promise<Plan> {
        const { priceText, features } = await this.root.evaluate((element) => {
            ...
            const features = [...element.querySelectorAll('li')].map((item) => item.textContent?.trim()).filter((item): item is string => Boolean(item));
    ...
    async expectVisible(): Promise<void> {
        ...
        await this.root.getByRole('heading', { level: 3, name: this.planName, exact: true }).waitFor({ state: 'visible' });
    }
