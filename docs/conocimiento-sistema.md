...
- **Reintegros — vista del tutor** (`/section/myreintegros`, `ReintegrosPage`, verificado en vivo 2026-08-29/30; distinto del backoffice Calidad/Finanzas documentado en `## Módulo: Reintegros` más abajo): secciones "Cuentas de acreditación" y "Mis reintegros", contadores de estado como texto (`"Pagado (N)"`/`"Solicitado (N)"`/`"Desaprobado (N)"`), botón "Acceder al historial" (deshabilitado si no hay historial). "Nuevo reintegro" abre un wizard de 3 pasos en `/section/nuevo-reintegro`.
  - **El wizard falla de forma reproducible en el selector de Mascota**, en **toda cuenta probada** (6 cuentas entre Vetify Adquirente y OSDE Capitado, incluida una con reintegros pagados reales). Causa raíz confirmada por inspección directa de red (`browser_network_requests`): `GET /api/bff/reintegros/mascotas` responde `400 {"code":"VAL-002","message":"Holder document must be a 7 or 8 digit DNI."}` para cualquier cuenta cuyo DNI no tenga 7-8 dígitos. **Es el mismo bug ya reportado en `docs/bugs/BUG-007-reintegros-dni-formato-invalido.md`, Jira `IMAS-4279`** (estado "En Progreso", Mariana Navarro) — no es un hallazgo nuevo, es la misma causa raíz repitiéndose en más cuentas.
