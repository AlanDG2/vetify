...
- **Reproducido también en mobile (app nativa Android), 2026-08-12, con una SEGUNDA cuenta distinta** (pool `VETIFY_ADQUIRENTE`, no la del reporte original): mismo síntoma de fondo, con una diferencia real de comportamiento entre plataformas — en mobile, la sección "Cuentas de acreditación" sí muestra el estado vacío silencioso ("Aún no tenés cuentas registradas", igual que Desktop), pero la sección de historial de reintegros muestra un **modal de error explícito** ("Algo salió mal / Probá de nuevo en un momento", `role="alertdialog"`) con el botón "Acceder al historial" deshabilitado — no el mismo estado vacío indistinguible que describe Desktop más arriba para esa parte. Confirma que el bug de backend es real y no está acotado a una sola cuenta, pero también sugiere que el manejo de error en mobile ya es parcialmente mejor que en Desktop para al menos una de las dos secciones (vale la pena que dev lo tenga en cuenta al corregir el punto "no distingue sin-datos de error-de-carga" de más abajo). No se investigó más a fondo el DNI de esta segunda cuenta ni se tocó Jira — queda como evidencia adicional, no como una segunda causa raíz distinta.
- **Reconfirmado 2026-08-28, 3 veces más, ahora también en el pool `OSDE_CAPITADO`** (antes solo se había visto en `VETIFY_ADQUIRENTE`): `user_1786584481760_8aea8baa@automation.com` (DNI `1588816521`, 10 dígitos) disparó el mismo `400 VAL-002` en `GET /api/reintegros/v1/mascotas` — **endpoint no listado arriba**, se suma a `cuentas-acreditacion`/`expedientes` como tercer endpoint afectado. Más tarde el mismo día, `user_1784732342305_69985b8e@automation.com` (DNI `167424201`, 9 dígitos) y `user_1784816945816_ece7183c@automation.com` (DNI `1913450260`, 10 dígitos) — ambas del pool `OSDE_CAPITADO` — reconfirmaron el mismo error en `cuentas-acreditacion`/`expedientes` (ver `docs/user-stories/IMAS-4052-workaround-reintegros-osde-capitado.tests.md`). Con esto son **5 cuentas independientes, en 2 pools distintos, con 3 endpoints afectados** — refuerza que es un bug transversal de validación en `reintegros-backend`, no un artefacto de una cuenta o segmento puntual.
